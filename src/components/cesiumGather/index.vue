<template>
    <div ref="cesiumContainer" class="cesium-ontainer">
        <!-- 有全屏时控制地图的内容放在cesiumContainer里面 -->
    </div>
</template>
<script setup>
import * as Cesium from 'cesium'
import 'cesium/Source/Widgets/widgets.css'
import { onMounted, shallowRef, inject, onUnmounted } from 'vue';

// import { CesiumRef, CESIUM_REF_KEY } from '@/libs/cesium/cesium-vue'

// import Jt from '@/libs/cesium/cesium-jt'
import cesiumCommonface from '@/libs/cesium/cesiumCommonface'
...
const init = () => {
    ...
    viewer.cesiumCommonface = new cesiumCommonface(viewer)
}
const initializeCesium = () => {
    ...
    // viewer.scene.skyAtmosphere.show = false//是否显示球边界
    // viewer.scene.globe.imageryLayers.get(0).alpha = 0.0;
    // viewer.scene.globe.baseColor = new Cesium.Color(0, 1, 0, 0); //默认为蓝色，这里改为绿色
    viewer.scene.globe.baseColor = Cesium.Color.fromCssColorString('#1e1e1e')

    // 设置为透明
    // viewer.scene.globe.showGroundAtmosphere = false;
    // viewer.scene.globe.baseColor = Cesium.Color.TRANSPARENT;
    // viewer.scene.globe.translucency.enabled = true;//设置为true时，球体被渲染为半透明的表面
    // 颜色--end
